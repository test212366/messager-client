...
const MessagesCom: FC<MessagesComProps> = ({ handlerSend, stop, handlerChange, persButton, handlerSelectPhoto, handlerPinnedChat, isRecord, onRecord, handlerSetPinnedChat, loadingMess, bottomScroll, chatUser, isPhone, localMess, pinnedChats, setLocalChats, setLoadingMess, setLocalMess, localChats, online, selectImage, setStateValues, stateValues, theme, user }) => {

	return (
		<div className="wrapper__home disFlex">

			<AllMessages chatUserLocal={stateValues} setChatUser={setStateValues} isPhone={isPhone} setLocalChats={setLocalChats} setLoadingMess={setLoadingMess} setLocalMessages={setLocalMess} userChatName={chatUser.user.name} localChats={localChats} yourName={user.user.name} pinnedChats={pinnedChats} />

			<div className={theme.isLigthTheme ? "chat light" : "chat"}>
				<CSSTransition in={stateValues.startAnim} classNames='headMess' timeout={800} >
					<div className={localMess.length === 0 && chatUser.user.name == '' ? theme.isLigthTheme ? 'user__head noneChat light' : 'user__head noneChat' : theme.isLigthTheme ? "user__head light" : "user__head"}>

						<div className="user__desc-all">
							{localMess.length === 0 && chatUser.user.name == '' ? <p className={theme.isLigthTheme ? "LogoRease black" : "LogoRease"}> <span>N</span>ZOx</p> : <>
								<NavLink to='/contacts' className='viewProfile'   >
									...
									<div className="user__description">
										<p className={theme.isLigthTheme ? "user__name black" : "user__name"}>{chatUser.user.name !== user.user.name ? chatUser.user.name : 'Вы'}</p>
										<p className={theme.isLigthTheme ? "user__online black" : "user__online"}> {online !== 'ne в ' ? `был(а) ${online}` : 'online'}</p>
										<p className={theme.isLigthTheme ? "user__status black" : "user__status"}>{chatUser.user.status}</p>
						...
						<div className="wrapper__navigation">
							{localMess.length === 0 && chatUser.user.name == '' ? '' : ""}
				...
				<div className="chat__messages">
					...
					{loadingMess.map((message: any, i: number) => <ChatMessage key={i} audioUrl={message.urlAudio} imgSRC={message.imgSRC} isRead={message.isRead} time={message.time} you={message.secondPartner === user.user.name} name={message.secondPartner} text={message.text} />)}
					{localMess.map((message: any, i: number) => <ChatMessage key={i} audioUrl={message.urlAudio} imgSRC={message.imgSRC} isRead={message.isRead} time={message.time} you={message.secondPartner === user.user.name} name={message.secondPartner} text={message.text} />)}
					<div className='scroll' ref={bottomScroll}></div>
					{localMess.length === 0 && chatUser.user.name == '' && !stateValues.loadingMessages ? <div className={theme.isLigthTheme ? "info light" : "info"}>
					...
					{localMess.length === 0 && chatUser.user.name != '' && loadingMess.length === 0 && !stateValues.loadingMessages ? <div className={theme.isLigthTheme ? "info light" : "info"}>
						<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50" width="35px" height="35px"><path d="M 25 2 C 12.309295 2 2 12.309295 2 25 C 2 37.690705 12.309295 48 25 48 C 37.690705 48 48 37.690705 48 25 C 48 12.309295 37.690705 2 25 2 z M 25 4 C 36.609824 4 46 13.390176 46 25 C 46 36.609824 36.609824 46 25 46 C 13.390176 46 4 36.609824 4 25 C 4 13.390176 13.390176 4 25 4 z M 25 11 A 3 3 0 0 0 22 14 A 3 3 0 0 0 25 17 A 3 3 0 0 0 28 14 A 3 3 0 0 0 25 11 z M 21 21 L 21 23 L 22 23 L 23 23 L 23 36 L 22 36 L 21 36 L 21 38 L 22 38 L 23 38 L 27 38 L 28 38 L 29 38 L 29 36 L 28 36 L 27 36 L 27 21 L 26 21 L 22 21 L 21 21 z" /></svg>
						Напишите что-нибудь..
					...
					{stateValues.typed ?
						...
						: ""}
					<CSSTransition in={stateValues.startAnim} classNames='input' timeout={800} >
			...
			<CSSTransition in={stateValues.showPersonalMan} classNames='showPersonalManager' timeout={300} unmountOnExit>
				<div className="pers">
					<div className="showPersonalManager-bg" onClick={() => setStateValues({ ...stateValues, showPersonalMan: !stateValues.showPersonalMan })} ></div>
					<PersonalManager />
				</div>

			</CSSTransition>
			{persButton ? <button className="showPersonalManager" onClick={() => setStateValues({ ...stateValues, showPersonalMan: !stateValues.showPersonalMan })}><div className={stateValues.showPersonalMan ? "arrow activeAr" : "arrow"}>
