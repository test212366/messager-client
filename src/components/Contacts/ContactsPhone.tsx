...
const ContactsPhone: FC<ConstactsPhoneProps> = ({ slider, theme, online, setStateValues, changeAvatar, stateValues, handlerSetStatusFetch, handlerEmitMessage, user, handlerFind, selectImage, handlerChangeInputValue, stateFind, handlerChangeContactUser, handlerChangeChatAllMessages, chatUser, handlerSetStatus }) => {
	return (
				...
				<div>
					<div className={`disFlex ${theme.isLigthTheme ? 'light' : ""}`}>
						...
						<div className={`messages__all contacts ${theme.isLigthTheme ? 'light' : ''}`}>
							...
							{stateFind.user.name ? <>
								...
								<div onClick={handlerChangeContactUser}>
									<CSSTransition in={stateValues.startAnim} classNames='messages' timeout={800}>
										<Message nameUser={stateFind.user.name} online={false} isContact={true} />
									</CSSTransition>
								</div>
							...
							{user.user.pinnedChats.map((chat: any, i: number) => <div className="temp" key={i} onClick={() => handlerChangeChatAllMessages(chat)}>
								<CSSTransition in={stateValues.startAnim} classNames='messages' timeout={800} >
							...
							{user.user.chats.map((chat: any, i: number) => <div key={i} className="temp" onClick={() => handlerChangeChatAllMessages(chat)} >
								<CSSTransition in={stateValues.startAnim} classNames='messages' timeout={800} >
				...
				<div>
					{
						user.user.name && chatUser.user.name === '' ?
							<>
								...
								<div className={`wrapper__contacts ${theme.isLigthTheme ? "light" : ""}`}>
									...
									<div className="user__desc-all contacts__userDesc">
										...
										<div className="user__description">
											...
											<p className={`user__online ${theme.isLigthTheme ? "black" : ""}`}>online </p>
...
l-14.484-14.484c-1.02-1.02-2.244-1.428-3.672-1.428C499.188,132.115,498.168,132.523,497.148,133.543z"/>
							...
							: <>
								...
								<div className={`wrapper__contacts ${theme.isLigthTheme ? "light" : ""}`}><CSSTransition in={stateValues.startAnim} classNames='headMess' timeout={800} >
									<div className={`user__head contacts__user-head ${theme.isLigthTheme ? "light" : ""}`}>
